...
fn render_snow_flake_side_single(p1x: f64, p1y: f64, p2x: f64, p2y: f64, n: i64,  arc: Arc<Mutex<RgbImage>>, pot4: i32){
    if n == 0{
        draw_line(&mut *(arc.lock().unwrap()), p1x as i64, p1y as i64, p2x as i64, p2y as i64);
    }
    else{
        let n2 = n - 1;
        let deltax = p2x - p1x;
        ...
        let to_pass = arc.clone(); render_snow_flake_side_single(p1x, p1y, mid1x, mid1y, n2, to_pass.clone(), pot4-1);
        let to_pass = arc.clone(); render_snow_flake_side_single(mid2x, mid2y, p2x, p2y, n2, to_pass.clone(), pot4-1);
        let to_pass = arc.clone(); render_snow_flake_side_single(mid1x, mid1y, heightx, heighty, n2, to_pass.clone(), pot4-1);
        let to_pass = arc.clone(); render_snow_flake_side_single(heightx, heighty, mid2x, mid2y, n2, to_pass.clone(), pot4-1);
        //
    }
}
