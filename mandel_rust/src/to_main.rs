...
// para processamento em paralelo
extern crate rayon;
use rayon::prelude::*;
...
fn belong_mandelbrot(max_iterations: u32, c: &Complex) -> bool {
...
fn main() {
    ...
    let mut img = RgbImage::new(img_side, img_side);

    let mut ppm = PPM::new(img_side, img_side);
    // Calculate for each pixel
        
    let mut guess_mandel = vec![false; img_side * img_side as usize]; 
